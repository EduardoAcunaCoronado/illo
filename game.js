...
const gamePauseClock = (() => {
    ...
    const createTimer = (callback, delay, args, interval) => {
        ...
        if (paused) {
            return interval ? nativeSetInterval(callback, delay, ...args) : nativeSetTimeout(callback, delay, ...args);
        }
        const id = nextId++;
    ...
    window.setTimeout = (callback, delay, ...args) => createTimer(callback, delay, args, false);
    window.setInterval = (callback, delay, ...args) => createTimer(callback, delay, args, true);
...
const canUseDevelopmentShortcuts = isLoopbackGame && (!window.desktopApp || window.desktopApp.isPackaged === false);

developmentShortcuts?.toggleAttribute('hidden', !canUseDevelopmentShortcuts);

if (toolsMenuLink && canUseDevelopmentShortcuts) {
    const loopbackHost = ['localhost', '127.0.0.1'].includes(location.hostname) ? location.hostname : 'localhost';
...
function updateRewindButton() {
    ...
    const visible = isGameRunning && engine.canRewind() && !cutsceneEnMarcha() && !hayElecciones;
...
function abrirMenuEscenas() {
    ...
    escenas.forEach((e) => {
        ...
        li.className = 'scenes-item' + (e.actual ? ' actual' : '') + (e.visitada && !e.actual ? ' visitada' : '');
        li.setAttribute('role', 'button');
        li.tabIndex = 0;
        if (e.actual) li.setAttribute('aria-current', 'true');
        li.innerHTML = '<span class="scenes-num">' + (e.index + 1) + '</span><span class="scenes-title"></span>';
    ...
    if (activo) {
        ...
        const centroActivo = activo.offsetTop - scenesList.offsetTop + activo.offsetHeight / 2;
        scenesList.scrollTop = Math.max(0, centroActivo - scenesList.clientHeight / 2);
    }
}
...
function storedTextSpeed() {
    ...
    return Number.isInteger(stored) && stored >= 0 && stored < TEXT_SPEED_OPTIONS.length ? stored : 2;
}

function applyTextSpeed(index) {
    const parsed = Number.parseInt(index, 10);
    const normalized = Number.isInteger(parsed) && parsed >= 0 && parsed < TEXT_SPEED_OPTIONS.length ? parsed : 2;
    ...
    const grupos = [
        {
            ...
            <div class="nm-setting-block nm-text-speed-setting">
                ...
                <div class="nm-text-speed-scale" aria-hidden="true">
                    ${TEXT_SPEED_OPTIONS.map(
                        (option, index) => `<span data-speed-step="${index}">${option.label}</span>`,
                    ).join('')}
                </div>
...
function wireSettings(panel) {
    ...
    const playTextPreview = () => {
        ...
        const typeNext = () => {
            ...
            previewTimer = setTimeout(typeNext, timing.delaysAfter[unitIndex] || 0);
        };
        typeNext();
    };
...
function loadGalleryManifest() {
    ...
    galleryManifestPromise = fetch(`${GALLERY_MANIFEST_SRC}?v=${Date.now()}`, { cache: 'no-store' })
        .then((response) => {
            if (!response.ok) {
...
function attachGalleryCleanSprites(manifest, cleanManifest) {
    ...
    for (const item of manifest.items || []) {
        ...
        for (const pose of item.poses || []) {
            ...
            if (Array.isArray(pose.animation?.frames)) {
                pose.animation.frames = pose.animation.frames.map((frame) => {
                    ...
                    return typeof frame === 'string' ? cleanedFrame : { ...frame, src: cleanedFrame };
                });
            }
        ...
        if (firstCleanPose && item.poses?.[0] === firstCleanPose.pose) {
            ...
            item.thumbnail =
                firstCleanPose.clean.galleryThumbnail || firstCleanPose.clean.thumbnail || firstCleanPose.clean.cleaned;
        }
    }
    return manifest;
}
...
function galleryFocusableElements(scope) {
    return [
        ...scope.querySelectorAll('button:not([disabled]), a[href], video[controls], [tabindex]:not([tabindex="-1"])'),
...
function showGalleryLoadError(error) {
  ...
  `;
    overlay.querySelector('.gallery-error-message').textContent = error?.message || 'No se ha podido leer el catálogo.';
...
async function showGalleryPanel() {
    if (document.getElementById('gallery-panel') || galleryBtn?.dataset.loading === 'true') {
        return;
    }
...
function renderGalleryPanel(manifest) {
  ...
  `;
    ...
    const categoryLabels = Object.fromEntries(manifest.categories.map((category) => [category.id, category.label]));
    ...
    function updateSpoilerToggle() {
        spoilerToggle.textContent = gallerySpoilersRevealed ? 'Ocultar spoilers' : 'Mostrar spoilers';
    ...
    function currentNavigableItems() {
        return state.filteredItems.filter((item) => gallerySpoilersRevealed || !item.spoiler);
    }
    ...
    function updateGalleryBlinkToggle(pose) {
        ...
        blinkToggleLabel.textContent = state.blinkEnabled ? 'Detener parpadeo' : 'Ver parpadeo';
    }
    ...
    function startGalleryBlink(pose, immediate = true) {
        ...
        let frameIndex = 0;
        const stillCurrent = () =>
            state.blinkEnabled && state.blinkRun === run && image.isConnected && currentLightboxPose() === pose;

        const scheduleBurst = (firstBurst = false) => {
            if (!stillCurrent()) return;
            const wait = firstBurst ? 320 : galleryBlinkDelay(animation.delayRange, 2400);
    ...
    function selectLightboxPose(item, index, focusSelected = false) {
        ...
        if (focusSelected) {
            focusGalleryElement(posePicker.querySelector(`.gallery-pose-option[data-pose-index="${index}"]`));
        }
    }
    ...
    function openLightbox(item) {
        state.returnFocus = grid.querySelector(`[data-gallery-id="${CSS.escape(item.id)}"]`) || document.activeElement;
    ...
    function createGalleryCard(item) {
        ...
        description.textContent = hiddenSpoiler ? 'Pulsa para decidir si quieres mostrarlo.' : item.description || '';
    ...
    overlay.addEventListener('keydown', (event) => {
        ...
        const scope = !spoilerDialog.hidden ? spoilerDialog : !lightbox.hidden ? lightbox : overlay;
...
function esPistaMusical(media) {
    ...
    const source = String(media._srcPath || media.currentSrc || media.src || '').replaceAll('\\', '/');
...
function setGamePaused(paused) {
    ...
    window.dispatchEvent(
        new CustomEvent('illo:pausechange', {
...
['pointerdown', 'pointerup', 'mousedown', 'mouseup', 'click', 'dblclick', 'wheel', 'touchstart', 'touchend'].forEach(
    (type) => {
...
function setupStartupSequence() {
    ...
    startupOpeningVideo.addEventListener('error', () => {
        startupOpeningStatus.textContent = 'No se ha podido cargar el opening. Pulsa «Saltar opening» para continuar.';
    });
...
function menuVideoEls() {
    return [document.getElementById('menu-video'), document.getElementById('menu-video-overlap')].filter(Boolean);
}
...
function playMenuChill() {
    ...
    const audio = engine.playSound(MENU_CHILL_SRC, {
        id: 'menu_chill',
        loop: true,
        volume: MENU_CHILL_VOL,
        fadeIn: 1800,
    });
    if (audio)
        audio.onerror = () => {
            try {
                engine.stopSound('menu_chill', 0);
            } catch (err) {}
        };
}
...
function playMenuTheme() {
    ...
    try {
        engine.stopSound('menu_chill', 800);
    } catch (err) {} // el tema manda
    const audio = engine.playSound(MENU_MUSIC_SRC, {
        id: 'menu_music',
        loop: false,
        volume: MENU_MUSIC_VOL,
        fadeIn: 600,
    });
...
function unlockMenuAudio(e) {
    ...
    if (document.body.classList.contains('startup-pending')) {
        if (e && e.target && e.target.closest && e.target.closest('#startup-enter')) menuAudioUnlocked = true;
        return;
    }
...
function stopMenuMedia() {
    try {
        engine.stopSound('menu_music', 700);
    } catch (err) {}
    try {
        engine.stopSound('menu_ambience', 700);
    } catch (err) {}
    try {
        engine.stopSound('menu_chill', 700);
    } catch (err) {}
...
async function loadAvailableChapters() {
    ...
    // Los capítulos son CONSECUTIVOS (chapter0, chapter1, ...). Sondeamos por bloques
    // para reducir latencia, pero preservamos el orden y la parada al primer 404/isFinal.
    for (let start = 0; start < 100; start += batchSize) {
        const batch = Array.from({ length: batchSize }, (_, offset) => {
            const index = start + offset;
            return { id: `chapter${index}`, index };
        });

        const chapters = await Promise.all(
            batch.map(({ id }) =>
                engine
                    .fetchJson(`chapters/${id}.json`, {
                        allow404: true,
                        fallback: null,
                    })
                    .catch(() => null),
        ...
        for (let offset = 0; offset < chapters.length; offset++) {
            const chapter = chapters[offset];
            const chapterId = batch[offset].id;
            ...
            const title = chapter.title || `Capítulo ${index}`;
            AVAILABLE_CHAPTERS.push({ id: chapterId, title });
            if (chapter.isFinal === true) {
                return AVAILABLE_CHAPTERS;
            }
        }
    }
...
async function loadAllCharacters() {
    ...
    const uniqueCharacters = [...new Set(PRELOADED_GAME_CHARACTERS)];
    const toLoad = uniqueCharacters.filter((character) => !engine.characters[engine.getCharacterKey(character)]);
    ...
    const loadResult = await Promise.all(toLoad.map((character) => engine.loadCharacter(character)));
    loadResult.forEach((character) => {
        if (!character) failed.push('missing');
    });

    if (failed.length > 0) {
        console.warn(`No se pudo precargar ${failed.length} personaje(s).`);
    }
}
...
async function checkChapterExists(chapterName) {
    const chapter = await engine.fetchJson(`chapters/${chapterName}.json`, {
        allow404: true,
        fallback: null,
    });
    return !!chapter;
...
async function loadGame() {
    if (document.getElementById('chapter-selector') || loadBtn.dataset.loading === 'true') {
        return;
    }
...
function showChapterSelector() {
    ...
    (selector.querySelector('.chapter-select-btn') || selector.querySelector('#chapter-selector-back'))?.focus();
}
