(function () {
    ...
    class KetchupMinigame {
        ...
        async play() {
            ...
            const startEnemyHp =
                this.options.startEnemyHp != null
                    ? Number(this.options.startEnemyHp)
                    : startAtHpRatio != null
                      ? enemyMaxHp * startAtHpRatio
                      : enemyMaxHp;
            ...
            const corruptKetchupIcon = cacheBust(
            ...
            const zipFloatingFrame = (index) =>
                cacheBust(`assets/images/characters/zip/ketchup/zip_flotando_${index}.webp`);
        ...
        `;
                ...
                const hitboxPartEntries = (hitbox) =>
                    Object.entries(hitbox?.parts || {}).length ? Object.entries(hitbox.parts) : [['base', hitbox]];
                ...
                const updateEnemyPos = () => {
                    ...
                    if (debugHitboxes) {
                        ...
                        bossHitboxes.replaceChildren(
                            ...Object.entries(profile.parts).map(([partId, part]) => {
                                ...
                                marker.style.borderRadius =
                                    part.shape === 'ellipse' || part.shape === 'circle' ? '50%' : '4px';
                                return marker;
                            }),
                        );
                    }
                ...
                const updateHud = () => {
                    ...
                    window.MinigameLifeDisplay.renderRepeated(livesEl, playerLives, playerMaxLives, '❤️');
                };
                    ...
                    window.HitboxDebugger.render({
                        ...
                        hitboxes: [
                            ...
                            ...shots.flatMap((shot, index) =>
                                hitboxPartEntries(hitboxConfig.shot).map(([partId, part]) => ({
                ...
                const makeSprite = (className, icon, x, y, size = 34, hitboxClass = '') => {
                    ...
                    if (debugHitboxes && hitboxClass) {
                        const config = hitboxClass.includes('is-chili')
                            ? hitboxConfig.shot
                            : hitboxClass.includes('is-blocking')
                              ? hitboxConfig.block
                              : hitboxConfig.hazard;
                ...
                const overlaps = (aX, aY, a, bX, bY, b, aRotation = 0, bRotation = 0) =>
                    hitboxParts(a).some((aPart) =>
                        hitboxParts(b).some((bPart) =>
                            polygonsOverlap(polygonFor(aX, aY, aPart, aRotation), polygonFor(bX, bY, bPart, bRotation)),
                        ),
                    );
                ...
                const loop = (time) => {
                    ...
                    if (!shouldFloat) {
                        ...
                        if (
                            !teleportActive &&
                            !specialWarningActive &&
                            !specialSequenceActive &&
                            !phaseAnimationActive &&
                            !phaseAnimationQueued
                        ) {
                            teleportTimer -= dt;
