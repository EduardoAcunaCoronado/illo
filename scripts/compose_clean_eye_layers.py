...
def build_pose(pose_key: str) -> dict:
    ...
    apply_matte(open_source, open_matte).save(open_path, "WEBP", lossless=True, method=6)
    apply_matte(closed_source, closed_matte).save(closed_path, "WEBP", lossless=True, method=6)
...
def main() -> None:
    ...
    for pose_key in pose_keys:
        ...
        print(f"{pose_key}: {result['eyesOpen']} | {result['eyesClosed']}")
