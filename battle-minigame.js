(function () {
            ...
            skills: [
                {
                    ...
                    target: 'enemy',
                    accuracy: 0.75,
                    description: 'Tiene un 75% de acierto antes de la evasión. Roba el objeto único del enemigo.',
                },
                {
                    id: 'prisa',
                    name: 'Prisa',
                ...
                },
                {
                    ...
                    power: 30,
                    target: 'enemy',
                    description: 'Un corte eléctrico rápido. Hace 30 de poder y no consume PM.',
                },
            ],
        },
        {
        ...
        },
        {
            ...
            skills: [
                ...
                },
                {
                    ...
                    duration: 2,
                    target: 'ally',
                    description: 'Aumenta el ataque del aliado elegido durante 2 turnos por 6 PM.',
                },
                {
                    id: 'provocar',
                    name: 'Provocar',
    ...
    class BattleMinigame {
        constructor(options = {}) {
            ...
            const enemyHp =
                Number.isFinite(configuredEnemyHp) && configuredEnemyHp > 0 ? configuredEnemyHp : this.enemyTemplate.hp;
            ...
            this.surviveTurns = Number(options.surviveTurns) > 0 ? Number(options.surviveTurns) : 0;
            this.turnsSurvived = 0;
            this.surviveWon = false;
            this.interludes = Array.isArray(options.interludes) ? options.interludes : [];
            // Grupo configurable: options.party = ["samu","edu"] limita los aliados.
            const partyIds = Array.isArray(options.party) && options.party.length ? options.party : null;
            const roster = partyIds ? ALLIES.filter((ally) => partyIds.includes(ally.id)) : ALLIES;
            ...
            this.allies = (roster.length ? roster : ALLIES).map((ally) => this.createFighter(conKit(ally), 'ally'));
        ...
        renderStatusIcons(fighter) {
            ...
            const isTauntTarget =
                ...
                this.enemy.statuses.some(
                    (status) => status.type === 'taunt' && status.targetId === fighter.id && status.turns > 0,
                );
            if (isTauntTarget) {
      ...
      `;
            activeName.style.setProperty('--ally-color', fighter.color || 'var(--battle-gold)');
        ...
        aplicarKosai(activo) {
            ...
            const listaALaVista = this.overlay.querySelectorAll('.battle-skill-list .battle-skill-button').length > 0;
            const enLaLista =
                listaALaVista &&
                !this.awaitingTarget &&
        ...
        `,
            ...
            description.textContent =
                this.battleInventory.length > 0 ? 'Selecciona un objeto para ver su efecto.' : 'No tienes objetos.';
        ...
        enableHealTargeting(actor, skill) {
            ...
            enemyEl?.addEventListener(
                ...
                (event) => {
                    ...
                    if (this.enemy.currentHp <= 0 || !this.awaitingTarget || this.targetSelectionToken !== token)
                        return;
                    clearTargeting();
                    this.performPlayerSkill(actor, skill, this.enemy);
                },
        ...
        consumeBattleItem(item) {
            const index = this.battleInventory.findIndex((inventoryItem) => inventoryItem.id === item.id);
      ...
      `;
        ...
        lockSkillPanel() {
            ...
            itemButton.textContent = 'Objetos';
            itemButton.onclick = null;
            this.overlay.querySelectorAll('.battle-skill-button').forEach((button) => {
                button.disabled = true;
            });
            this.overlay.querySelector('.battle-skill-description').textContent = 'Espera al siguiente turno aliado.';
        }
        ...
        async useEnemyNormalSkill(actor) {
            ...
            const skill = available.length > 0 ? available[Math.floor(Math.random() * available.length)] : free[0];
            actor.currentPm = Math.max(0, actor.currentPm - skill.pmCost);
        ...
        applySkill(actor, skill, target) {
            if (skill.target === 'all_allies' || skill.type === 'party_heal') {
                ...
                let enemyDamage = results.reduce((sum, result) => sum + (result.enemyDamage || 0), 0);
                let extraText = '';
        ...
        applySingleTargetSkill(actor, skill, target) {
            ...
            if (
                ['damage', 'damage_speed_down', 'mp_drain', 'mp_damage', 'damage_attack_down', 'multi_damage'].includes(
                    skill.type,
                )
            ) {
        ...
        applyItem(item, target) {
            if (item.type === 'diapason_resonance') {
                const fallen = this.allies.filter((ally) => ally.currentHp <= 0).slice(0, 3);
                ...
                const canReachAiri = ['amalgama', 'amalgama_final'].includes(this.enemy.id);
                if (canReachAiri) {
                    ...
                    return {
                        text: 'La nota comprime el ruido. El núcleo azul queda expuesto y Airi responde desde dentro: abrirá una brecha cada dos turnos enemigos.',
                        consumed: true,
                    };
                }

                return {
                    text: 'La nota limpia levanta al grupo, pero aquí no encuentra ningún núcleo al que responder.',
                    consumed: true,
                };
            }
            ...
            if (item.type === 'revive_three') {
                ...
                if (recoveredHp > 0 || recoveredPm > 0) {
                    effects.push(`el grupo recupera ${recoveredHp} HP y ${recoveredPm} PM en total`);
                }
        ...
        showAllyDamageNumbers(damages = []) {
            damages.forEach(({ id, damage }) => {
                ...
                const allyEl = this.overlay.querySelector(`.battle-ally-card[data-ally="${id}"]`);
                if (!allyEl) return;
                ...
                const left = anchorRect.left - overlayRect.left + anchorRect.width * (0.45 + Math.random() * 0.18);
                const top = anchorRect.top - overlayRect.top + anchorRect.height * (0.04 + Math.random() * 0.08);
        ...
        isAmalgamaFinalAttack(actor, skill) {
            return skill?.isFinalAttack && ['amalgama', 'amalgama_final'].includes(actor?.id);
        }
        ...
        rollHit(actor, skill, target) {
            ...
            const hitChance = clamp(baseAccuracy - this.getEvasion(target), 0, 1);
        ...
        updateSurviveHud() {
            ...
            this.surviveChip.innerHTML = left > 0 ? '🛡️ AGUANTAD LA PUERTA' : '🎤 ¡YA CANTA!';
        ...
        showBattleResult(won) {
            const winTitle = this.options.victoryTitle || (this.surviveTurns > 0 ? '¡Habéis aguantado!' : 'Victoria');
            const winText =
                this.options.victoryText ||
                (this.surviveTurns > 0
                    ? 'El escenario se enciende a vuestra espalda.'
                    : `${this.enemy.name} ha sido derrotada.`);
            const loseText = this.options.defeatText || 'El equipo ha caído en combate.';
